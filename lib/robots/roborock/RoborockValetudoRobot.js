...
class RoborockValetudoRobot extends MiioValetudoRobot {
    ...
    constructor(options) {
        ...
        this.registerCapability(new capabilities.RoborockSpeakerVolumeControlCapability({
            robot: this
        }));
        this.registerCapability(new capabilities.RoborockSpeakerTestCapability({
            robot: this
        }));
        this.registerCapability(new capabilities.RoborockSpotCleaningCapability({
            robot: this
        }));
        this.registerCapability(new capabilities.RoborockJobHistoryCapability({
            robot: this
        }));
    }
    ...
    parseMap(data) {
        if (this.triggerSnapshot) {
            this.snapshot.map = RRMapParser.PARSE(data) || require("../../res/default_map");
            this.triggerSnapshot = false;
        } else {
            this.state.map = RRMapParser.PARSE(data) || require("../../res/default_map");
        }
    ...
    initSnapshot() {
        this.snapshot = new entities.state.RobotState({
    ...
    getErrorCodeDescription(error) {
        return GET_ERROR_CODE_DESCRIPTION(error);
    }
}
